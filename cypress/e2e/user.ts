...
describe('User', () => {
  ...
  it('should sign in and sign out', () => {
    ...
    cy.findByText(/rodrigo10/i).should('exist').click()
    cy.findByText(/sign out/i).click()

    cy.findByRole('link', { name: /sign in/i }).should('exist')
    cy.findByText(/rodrigo10/i).should('not.exist')
  });
  ...
  it('should sign the user and redirect to the page that it was previously defined', () => {
    ...
    cy.findByLabelText(/username/i).should('have.value', 'rodrigo10')
    cy.findByLabelText(/e-mail/i).should('have.value', 'rodrigorcfernandes+10@gmail.com')
  });
});